...
The ``account-token-position.conf`` rows are a service behind an export
that inserts the caller's account token (``account_token_position``): a
hand-rolled responder on ``svc.*.prompt`` verifies with
