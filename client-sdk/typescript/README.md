...
- `tools` offers fewer than the six. Each definition costs input tokens on every model call, about 1.5k for all six, so an agent that needs no async calls offers three: `new AgentTools({ agents, tools: BLOCKING_AGENT_TOOLS })`, which are `discover_agents`, `prompt_agent` and `answer_agent`. `AGENT_TOOL_NAMES` names all six. Without `wait_agent` nothing is detached: `prompt_agent` and `answer_agent` lose their `wait` parameter, and their descriptions read blocking-only words.
