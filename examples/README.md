...
| `python-identity-workbook/` | agent + caller | Python + `synadia-ai-agents` + `synadia-ai-agent-service` | End-to-end sender identity with three local NKEY users: signed Echo and Hello registrations that accept prompts with or without sender identity, plus a client-SDK CLI that demonstrates both modes. Includes a real-NATS e2e test and uses editable monorepo SDK sources until the identity-bearing releases reach PyPI. |
