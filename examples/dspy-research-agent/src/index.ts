...
// calls, and a pluggable `web.*` tool group (search + page extract). Exposed
// as a NATS Agent Protocol service; streams REPL turn activity as status
...
const scrubbedFetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
  ...
  if (DEBUG_TRACE && finalInit?.body) {
    await fs.writeFile(`/tmp/research-req-${n}.json`, String(finalInit.body));
  }
  ...
  if (DEBUG_TRACE) {
    await fs.writeFile(`/tmp/research-res-${n}.json`, JSON.stringify(body, null, 2));
  }
...
const agentService = new ReferenceAgent({
  nc,
  agent: "research",
  ...
  promptHandler: async (msg: ServiceMsg) => {
    ...
    chunk("status", "ack");
    chunk("status", `provider: ${searchProvider.name}`);

    const emit = (line: string): void => chunk("status", line);
    ...
    try {
      ...
      if (result.report) chunk("response", String(result.report));
      const citations = Array.isArray(result.citations) ? result.citations : [];
      if (citations.length > 0) {
        chunk("response", "\n\n---\n**Sources**\n");
        citations.forEach((u, i) => chunk("response", `\n[${i + 1}] ${u}`));
      }
    } catch (err) {
...
console.log(`research agent listening on ${agentService.promptSubject}`);
