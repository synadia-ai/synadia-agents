...
import type { DiscoveredAgentDTO, PiExecSessionSummary } from "../../wire.ts";
...
const rows = computed(() =>
  visibleSessions.value.map((agent) => {
    const summary = piexecState.summaries.get(agent.name);
...
async function refresh(): Promise<void> {
  ...
  if (!controller || piexecState.refreshing) return;
  piexecState.refreshing = true;
  try {
    const list = await bridge.piexecList(controller.instanceId);
    mergeSummaries(list);
  } catch (err) {
    piexecState.lastError = `list failed: ${(err as Error).message}`;
  } finally {
    piexecState.refreshing = false;
  }
}
...
async function onStop(agent: DiscoveredAgentDTO): Promise<void> {
  ...
  try {
    await bridge.piexecStop(controller.instanceId, agent.name);
    onStopped(agent.name);
  } catch (err) {
    piexecState.lastError = `stop failed: ${(err as Error).message}`;
  } finally {
...
function fmtRemaining(summary: PiExecSessionSummary | undefined): string {
  if (!summary) return "";
...
function lifetimePercent(summary: PiExecSessionSummary | undefined): number {
  ...
  <section class="panel">
    <div class="head">
      ...
      <button
        ...
        :disabled="piexecState.refreshing || !selectedController"
        @click="refresh"
      >{{ piexecState.refreshing ? '…' : '↻' }}</button>
    </div>
    <div v-if="!selectedController" class="empty mono">select a controller</div>
      ...
      >
        ...
        >
          ...
          <div class="meta">
            ...
            <span v-if="summary?.thinking_level" class="meta-key mono">think</span>
            <span v-if="summary?.thinking_level" class="meta-val mono">{{ summary.thinking_level }}</span>
