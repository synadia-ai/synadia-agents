// The extension point: naming and precedence with OpenClaw's variables,
...
describe("resolveExtensionEntries: the variables and the config field", () => {
  ...
  it("SYNADIA_OPENCLAW_EXTENSIONS wins over SYNADIA_AGENT_EXTENSIONS", () => {
    const resolved = resolveExtensionEntries(["from-config"], {
      [OPENCLAW_EXTENSIONS_VAR]: "openclaw-only",
      [AGENT_EXTENSIONS_VAR]: "shared",
    });
    expect(resolved.source).toBe(OPENCLAW_EXTENSIONS_VAR);
    expect(resolved.entries).toEqual([{ module: "openclaw-only", options: {} }]);
  });
...
describe("loadExtensions", () => {
  let dir: string;
  beforeAll(() => {
    dir = mkdtempSync(join(tmpdir(), "openclaw-ext-test-"));
    writeFileSync(
      join(dir, "good.mjs"),
  ...
  it("an absolute file path loads; the factory gets the context with the entry's options", async () => {
    ...
    expect(seen.harness).toBe("openclaw");
  ...
  it("a missing module is skipped with one log line", async () => {
    ...
    const result = await loadExtensions(
      [
        ...
        { module: "no-such-package-for-the-openclaw-channel-test", options: {} },
    ...
    expect(logger.lines[1]).toMatch(
      /^error extension "no-such-package-for-the-openclaw-channel-test" not loaded: /,
    );
...
describe("composeExtensions: OpenClaw's events, fail-open", () => {
  ...
  it("aroundDispatch: the wrapper's context is what the dispatch sees, and its promise comes back", async () => {
    const logger = recordingLogger();
    const als = new AsyncLocalStorage<string>();
    const composed = composeExtensions(
      [
        loaded("binder", {
          events: { aroundDispatch: (r, run) => als.run(`bound:${r.id}`, run) },
        }),
      ],
      logger,
    );
    const dispatch = Promise.resolve("dispatched");
    let seen: string | undefined;
    const got = composed.events.aroundDispatch(request, () => {
      seen = als.getStore();
      return dispatch;
    });
    // run's own promise, not a wrapper's around it.
    expect(got).toBe(dispatch);
    expect(seen).toBe("bound:7");
    expect(await got).toBe("dispatched");
    expect(logger.lines).toEqual([]);
  });

  it("aroundDispatch wrappers nest with the first loaded outermost, and the dispatch runs once", async () => {
    const order: string[] = [];
    const composed = composeExtensions(
      [
        loaded("outer", {
          events: {
            async aroundDispatch(_r, run) {
              order.push("outer>");
              const v = await run();
              order.push("<outer");
              return v;
    ...
    let runs = 0;
    const value = await composed.events.aroundDispatch(request, async () => {
      order.push("dispatch");
      runs++;
      return "done";
    });
    ...
    expect(runs).toBe(1);
    await delayTicks(4);
    expect(order).toEqual(["outer>", "inner>", "dispatch", "<inner", "<outer"]);
  });

  it("a dispatch wrapper that does not call run, or throws before it, has run called by the plugin, logged once", async () => {
    const logger = recordingLogger();
    let runs = 0;
    const composed = composeExtensions(
      [
        loaded("forgets", { events: { aroundDispatch: (() => Promise.resolve()) as never } }),
        loaded("throws-first", {
          events: {
            aroundDispatch() {
    ...
    expect(await composed.events.aroundDispatch(request, async () => ++runs)).toBe(1);
    expect(await composed.events.aroundDispatch(request, async () => ++runs)).toBe(2);
    expect(runs).toBe(2);
    expect(logger.lines).toEqual([
      'warn extension "forgets" aroundDispatch returned without calling run; the plugin ran the step itself',
      'warn extension "throws-first" aroundDispatch failed: before run',
    ]);
  });

  it("the dispatch's own rejection reaches the plugin even through a wrapper that swallows it", async () => {
    const logger = recordingLogger();
    const composed = composeExtensions(
      [
        loaded("swallows", {
    ...
    await expect(
      composed.events.aroundDispatch(request, () => Promise.reject(new Error("OpenClaw refused"))),
    ).rejects.toThrow("OpenClaw refused");
    await delayTicks(4);
  ...
  it("an async wrapper that throws after run is logged once and never becomes an unhandled rejection", async () => {
    ...
    try {
      ...
      expect(await composed.events.aroundDispatch(request, async () => "ok")).toBe("ok");
      expect(await composed.events.aroundDispatch(request, async () => "ok")).toBe("ok");
      await new Promise((resolve) => setTimeout(resolve, 10));
      expect(unhandled).toEqual([]);
      expect(logger.lines).toEqual(['warn extension "throws-after" aroundDispatch failed: after run']);
      // The dispatch's own rejection, passed through by such a wrapper, is not blamed on it.
      await expect(
        composed.events.aroundDispatch(request, () => Promise.reject(new Error("own"))),
      ).rejects.toThrow("own");
      await new Promise((resolve) => setTimeout(resolve, 10));
      expect(unhandled).toEqual([]);
      expect(logger.lines).toHaveLength(1);
  ...
  it("an async wrapper that calls run only after an await: the plugin runs the step once, the late call is ignored", async () => {
    ...
    const composed = composeExtensions(
      [
        loaded("late", {
          events: {
            aroundDispatch: (async (_r: unknown, run: () => Promise<unknown>) => {
    ...
    expect(runs).toBe(1);
    expect(logger.lines).toEqual([
      'warn extension "late" aroundDispatch returned without calling run; the plugin ran the step itself',
      'warn extension "late" aroundDispatch called run more than once, or after the plugin had run the step itself; the extra call was ignored',
    ]);
  });
  ...
  it("with no extension, every event is a no-op and every wrapper just runs its step", async () => {
    ...
    expect(await composed.events.aroundDispatch(request, async () => "ran")).toBe("ran");
