#!/usr/bin/env bun
// CLI entrypoint for the gemini NATS channel.
...
function parseArgs(argv: ReadonlyArray<string>): ParsedArgs {
  ...
  const owner =
    out["owner"] ??
    process.env["GEMINI_AGENT_OWNER"] ??
    config.owner ??
    process.env["USER"] ??
    "anon";
  const session =
    out["session"] ??
    process.env["GEMINI_AGENT_SESSION"] ??
    config.session ??
    "default";
  const cwd =
    out["cwd"] ??
    process.env["GEMINI_AGENT_CWD"] ??
    join(tmpdir(), "gemini-agent", session);
