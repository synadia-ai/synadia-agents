...
- **`opencode/`** - planned TypeScript/Bun adapter for OpenCode server/session surfaces. Built on `@synadia-ai/agent-service`; the current scaffold has config, CLI/doctor, metadata, and test seams but intentionally refuses to serve prompts until the real OpenCode SDK lifecycle/event/permission bridge lands.
