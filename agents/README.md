...
| `opencode/`         | `opencode` | [OpenCode](https://opencode.ai/) server/session | `agents.prompt.opencode.<owner>.<session>` | server-negotiated | false |
...
- **`opencode/`** - TypeScript/Bun adapter for OpenCode server/session surfaces. Built on `@synadia-ai/agent-service`; supports managed `opencode serve` lifecycle, attached `--base-url` mode, prompt routing through `@opencode-ai/sdk`, SSE response chunk mapping, and permission events bridged to protocol `query` chunks when `permission_policy=query`. v1 advertises `attachments_ok=false` and rejects attachment envelopes until OpenCode file ingestion is mapped end-to-end.
