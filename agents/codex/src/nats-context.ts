// NATS connection resolution for the codex channel.
//
// Resolution order (first hit wins):
//   1. `--nats-url` flag / `NATS_URL` env (direct URL)
//   2. `--nats-context` flag (saved `nats` CLI context by name)
//   3. `~/.codex/agent/nats-channel.json` (optional config file)
...
//   }
//
// Only `context` is consumed here; `owner` / `session` are read by the CLI
// to seed defaults if no flag / env override was provided.
...
export const CONFIG_DIR = join(homedir(), ".codex", "agent");
