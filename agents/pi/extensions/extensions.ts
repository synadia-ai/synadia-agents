/**
 * The extension point of the PI channel.
 *
 * An extension is a module the operator names in `SYNADIA_PI_EXTENSIONS`,
 * `SYNADIA_AGENT_EXTENSIONS` or the `extensions` field of
 * `nats-channel.json`. It is loaded once at start, before the channel
 ...
 * handlers for PI's events. The contract an extension author reads is
...
/** The harnesses that expose this extension point. PI is this plugin's. */
...
export interface AgentExtensionSettings {
  ...
  /**
   * The name token as configured. The channel checks it for a collision
   * once on the bus and may suffix it (`-2`, `-3`, …); the definitive
   * address is `service.subject.prompt` in `started()`.
   */
...
export interface HarnessEvents {
  ...
  // PI.
  /** Around the hand-over of the prompt to PI's loop (`pi.sendUserMessage()`). */
  aroundInject?<T>(request: ServedRequest, run: () => T): T;
  /** Headers to add to a provider request while `request` is PI's active turn. */
  providerHeaders?(
...
export const PI_EXTENSIONS_VAR = "SYNADIA_PI_EXTENSIONS";
export const AGENT_EXTENSIONS_VAR = "SYNADIA_AGENT_EXTENSIONS";

/** One entry of the `extensions` array in `nats-channel.json`. */
 ...
 *
 * Precedence: `SYNADIA_PI_EXTENSIONS`, then `SYNADIA_AGENT_EXTENSIONS`,
...
export function resolveExtensionEntries(
  ...
  for (const variable of [PI_EXTENSIONS_VAR, AGENT_EXTENSIONS_VAR] as const) {
...
// ─────────────────────────────────────────────────────────────────────────────
// Composition: one set of hooks, one fail-open dispatcher for PI's events
// ─────────────────────────────────────────────────────────────────────────────
...
export interface PiHarnessEventDispatch {
  promptAccepted(request: ServedRequest): void;
  promptEnded(request: ServedRequest, outcome: Outcome, atMs: number): void;
  aroundInject<T>(request: ServedRequest, run: () => T): T;
  /** The extensions' headers merged in load order; `{}` when none. */
  providerHeaders(request: ServedRequest): Record<string, string>;
  aroundToolCall<T>(
    request: ServedRequest | undefined,
    toolName: string,
    run: () => T,
...
export interface ComposedExtensions {
  ...
  readonly events: PiHarnessEventDispatch;
}
...
export function composeExtensions(
   ...
   */
  const guarded = <T>(
    event: "aroundInject" | "aroundToolCall",
    l: LoadedExtension,
    invoke: (inner: () => T) => T,
    inner: () => T,
  ): T => {
  ...
  return {
    ...
    events: {
      ...
      aroundInject(request, run) {
        return wrapAll(
          "aroundInject",
          (l, inner) => l.extension.events!.aroundInject!(request, inner),
          run,
        );
      },
      providerHeaders(request) {
        const merged: Record<string, string> = {};
        for (const l of withEvent("providerHeaders")) {
          try {
            const headers = l.extension.events!.providerHeaders!(request);
            if (!headers) continue;
            ...
            logOnce(
              `${l.name}:providerHeaders`,
              `extension "${l.name}" providerHeaders failed: ${errorText(e)}`,
            );
          }
        }
        return merged;
