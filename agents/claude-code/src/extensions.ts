/**
 * The extension point of the Claude Code channel.
 *
 * An extension is a module the operator names in
 * `SYNADIA_CLAUDE_CODE_EXTENSIONS`, `SYNADIA_AGENT_EXTENSIONS` or the
 * `extensions` field of `config.json` in the channel's state directory. It
 * is loaded once when the MCP server starts, before the channel connects,
 * and hands the channel what the SDK's hooks accept (prompt and request
 * interceptors, heartbeat extras, agent-tools extensions) plus handlers for
 * the session's events. The contract an extension author reads is
 * `agents/EXTENSIONS.md`; this file is its reference in code.
 *
 * This file is the third copy of one contract, kept in step with
 * `agents/openclaw/src/extensions.ts` and `agents/pi/extensions/extensions.ts`:
 * the types, the naming, the loader and the fail-open composition are the
 * same code, and only the dispatch of the harness's events differs (here
 * Claude Code's `sessionStarted` and `turnStopped`, and no dispatch
 * wrapper). A change to the shared parts is made in all three until they
 * get a shared home.
 *
 * The bundle (`runtime/server.js`) inlines both SDKs, but not an extension:
 * the specifier handed to `import()` is a runtime value, so the bundler
 * leaves the import to the runtime, and an extension always runs on its
 * own installed copy of the SDK packages.
 *
 ...
 *     the session's events. Every event call fails open: a throw is logged
...
/** The harnesses that expose this extension point. Claude Code is this plugin's. */
...
export interface AgentExtensionSettings {
  ...
  /**
   * The name token as configured (the session name). The channel checks it
   * for a collision once on the bus and may suffix it (`-2`, `-3`, …); the
   * definitive address is `service.subject.prompt` in `started()`.
   */
 ...
 * Claude Code calls `promptAccepted`, `promptEnded`, `aroundToolCall`,
 * `sessionStarted` and `turnStopped`; the other harnesses' events are
 * listed so one shape serves all three.
 */
export interface HarnessEvents {
  ...
  // Claude Code.
  /**
   * The plugin's `SessionStart` hook ran: `source` as Claude Code gave it
   * (`startup`, `resume`, `clear`, `compact`). Called once at start for the
   * session the hook recorded before the server came up.
   */
  sessionStarted?(sessionId: string, source: string): void;
  /** The plugin's `Stop` hook ran: the turn ended at `atMs` (epoch ms, the hook's clock). */
...
export const CLAUDE_CODE_EXTENSIONS_VAR = "SYNADIA_CLAUDE_CODE_EXTENSIONS";
export const AGENT_EXTENSIONS_VAR = "SYNADIA_AGENT_EXTENSIONS";

/** One entry of `config.json`'s `extensions` array. */
...
export type ExtensionSource =
  | typeof CLAUDE_CODE_EXTENSIONS_VAR
 ...
 *
 * Precedence: `SYNADIA_CLAUDE_CODE_EXTENSIONS`, then `SYNADIA_AGENT_EXTENSIONS`,
...
export function resolveExtensionEntries(
  ...
  for (const variable of [CLAUDE_CODE_EXTENSIONS_VAR, AGENT_EXTENSIONS_VAR] as const) {
...
export interface ComposedExtensions {
  ...
  readonly events: ClaudeCodeHarnessEventDispatch;
}
...
type WrapperEvent = "aroundToolCall";
...
export function composeExtensions(
  ...
  const notify = <
    K extends "promptAccepted" | "promptEnded" | "sessionStarted" | "turnStopped",
  >(
    event: K,
  ...
  return {
    ...
    events: {
      ...
      sessionStarted(sessionId, source) {
        notify("sessionStarted", (handler) => handler(sessionId, source));
      },
      turnStopped(sessionId, atMs) {
        notify("turnStopped", (handler) => handler(sessionId, atMs));
      },
    },
  };
}
